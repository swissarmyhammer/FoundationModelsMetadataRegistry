...
// (plan.md §5 "Port, don't depend"). The two field-weight constants are
// renamed for this domain's `SearchableMetadata` fields (plan.md "Catalog
// contract": `id` + rendered `block`): CodeContextKit's
// `symbolPathFieldWeight` -> `idFieldWeight`, `bodyFieldWeight` ->
// `blockFieldWeight`. No other behavior changes.
...
public enum BM25 {
    ...
    /// Field weight applied to `id` occurrences when computing a
    /// document's weighted term frequency — five times `blockFieldWeight`,
    /// so an id match dominates a block-only match for the same term.
    public static let idFieldWeight: Double = 5.0

    /// Field weight applied to `block` (the rendered `renderBlock()`
    /// output) occurrences when computing a document's weighted term
    /// frequency; the baseline weight.
    public static let blockFieldWeight: Double = 1.0
}
...
public struct BM25Corpus: Sendable {
    ...
    /// `BM25.idFieldWeight` / `BM25.blockFieldWeight`) over each
