...
// `swiftpm-test-gpu-metallib-limit`), by way of `FoundationModelsMultitool`'s
// own port (`IntegrationTests/.../Support/MetalLibraryTestBootstrap.swift`) —
// this is the same `Cmlx`/`mlx-swift` dependency, so the same fix applies
// verbatim.
//
...
// integration scenarios hit under `swift test --package-path
// IntegrationTests`: the first live `MLXLanguageModel`/Router generation
// call touches the GPU device.
//
// Confirmed by direct measurement: at swissarmyhammer/workflows commit
// 0580114 the shared `swift-ci.yaml` workflow's `integration-metallib-glob`
// colocation step copied the found metallib beside the `.xctest` binary
// WITHOUT renaming it (it kept the source filename, `default.metallib`),
// which satisfied none of the five probes above -- probe (1)/(2) need the
// literal name `mlx.metallib`, and probe (4) needs the file inside a
// `Resources/` subdirectory. workflows commit 20c0a0a fixed that step to
// copy both names. This bootstrap stays regardless: it is the root-cause fix
// (works under any CI, or none, and does not depend on a shared workflow's
// exact colocation behavior), and the CI step is defense-in-depth on top of
