...
// budget, forcing the over-budget one-prompt-per-run path (plan.md §6). Both
// paths are GPU-free and need no network -- each one-off session is a
// scripted `DemoAgentSession`.
...
let selected = try await runBigCatalogOverBudgetSelection(catalog: catalog, query: bigCatalogNeedleQuery)
