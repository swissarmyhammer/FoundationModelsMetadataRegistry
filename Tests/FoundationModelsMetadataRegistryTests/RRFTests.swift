...
/// Reciprocal Rank Fusion and `Hit`/`Signals` plumbing tests.
///
/// Adapted from CodeContextKit's `RankerTests.swift` "RRF" and "Hit /
/// Signals" sections (which port the Rust `swissarmyhammer-search` crate's
/// `score.rs`/`lib.rs` test suites; see plan.md "Search"). Covers the RRF
/// acceptance criteria for this port: `k = 60`, 0-based ranks, an absent
/// signal contributing nothing (never zero-filled), and normalized scores
/// staying in `[0, 1]`.
struct RRFTests {
