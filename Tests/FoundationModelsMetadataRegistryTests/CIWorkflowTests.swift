...
///
/// The org test contract (swissarmyhammer/workflows' README) says CI runs
/// the unit job before the integration job. Both jobs use one runner label,
...
/// can run first while the fast unit signal waits. This suite makes sure
/// the `integration` job declares `needs: unit`, so a later edit cannot
/// remove the edge without a red unit test. Mirrors Multitool's own
/// `CIWorkflowTests`.
@Suite("CI workflow")
